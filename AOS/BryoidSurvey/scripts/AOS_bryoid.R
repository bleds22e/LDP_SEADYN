...
con <- file("./AOS/BryoidSurvey/raw_data/txt_files/earlier_years/aos81.b.st25.txt")
...
con <- file("./AOS/BryoidSurvey/raw_data/txt_files/earlier_years/aos81.b.sx25.txt")
...
bc_1981x <- cbind(quad.info, bc_1981) %>% mutate(stand_size = 25)
...
#write_csv(taxonomy.1981, "./AOS/BryoidSurvey/metadata/sp_codes_1981.csv")
write_csv(bc_1981x, "./AOS/BryoidSurvey/raw_data/csv_files/AOS_bryoid_1981.csv")

# 2: 1982 25 m2 plot bryoid data: has implicit zeroes

con <- file("./AOS/BryoidSurvey/raw_data/txt_files/earlier_years/aos82.b.st25.txt")
...
con <- file("./AOS/BryoidSurvey/raw_data/txt_files/earlier_years/aos82.b.sp25.txt")
...
#write_csv(taxonomy.1982, "./AOS/BryoidSurvey/metadata/sp_codes_1982.csv")
write_csv(bc_1982x, "./AOS/BryoidSurvey/raw_data/csv_files/AOS_bryoid_1982.csv")

# 3: 1983 25 m2 bryoid data

con <- file("./AOS/BryoidSurvey/raw_data/txt_files/earlier_years/aos83.b.st25.txt")
...
con <- file("./AOS/BryoidSurvey/raw_data/txt_files/earlier_years/aos83.b.sx25.txt")
...
quad.info <- cbind(quad.sub, stand) %>% rename(stand = 2) %>% 
  mutate(year = 1983, month = 8, stand_size = 25)
...
#write_csv(taxonomy.1983, "./AOS/BryoidSurvey/metadata/sp_codes_1983.csv")
write_csv(bc_1983x, "./AOS/BryoidSurvey/raw_data/csv_files/AOS_bryoid_1983.csv")

# 4: 0.5 x 0.5 m microplots, 1983 & 1984

# list all the file names
file.list <- list.files("./AOS/BryoidSurvey/raw_data/txt_files/later_years/data")
...
for (file in 1:length(file.list)){
  ...
  con <- file(paste("./AOS/BryoidSurvey/raw_data/txt_files/later_years/data/", filename, sep = ""))
  open(con)
  results_list <- list()
  current_line <- 1
  while (length(line <- readLines(con, n = 1, warn = FALSE)) > 0){
...
survey_dates <- read_csv("./AOS/BryoidSurvey/metadata/AOS_BryoidSurveyDates.csv") %>% 
...
quads_1983 <- all_quads3 %>% filter(year == "1983") %>% mutate(stand_size = 5)
quads_1984 <- all_quads3 %>% filter(year == "1984") %>% mutate(stand_size = 5)

## extract the metadata to give dfs dates and sp codes

# 1983 metadata

con <- file("./AOS/BryoidSurvey/raw_data/txt_files/later_years/metadata/aos83.b.sx.5.txt")
open(con)
results_list <- list()
current_line <- 1
while (length(line <- readLines(con, n = 1, warn = FALSE)) > 0){
...
con <- file("./AOS/BryoidSurvey/raw_data/txt_files/later_years/metadata/aos84.b.sx.5.txt")
open(con)
results_list <- list()
current_line <- 1
while (length(line <- readLines(con, n = 1, warn = FALSE)) > 0){
...
#write_csv(taxonomy.1984, "./AOS/BryoidSurvey/metadata/sp_codes_1984.csv")
...
write_csv(aos_bc_1983, "./AOS/BryoidSurvey/raw_data/csv_files/AOS_bryoid_1983.csv")
write_csv(aos_bc_1984, "./AOS/BryoidSurvey/raw_data/csv_files/AOS_bryoid_1984.csv")

# now need to put together all of these dataframes

file.list <- list.files("./AOS/BryoidSurvey/metadata", pattern = "sp_codes")

all_taxa <-data.frame() # create an empty dataframe for storing all taxonomic information

for (file in 1:length(file.list)){
  df <- read_csv(paste("./AOS/BryoidSurvey/metadata/", file.list[file], sep = ""), col_names = F)
...
#write_csv(all.sp.codes.taxonomy, "./AOS/BryoidSurvey/metadata/AOS_BryoidSpList.csv")

# finally, to join together the data

file.list <- list.files("./AOS/BryoidSurvey/raw_data/csv_files", pattern = "*.csv")
...
all_bc <- all_bc %>%  relocate(stand,year,month,day,stand_size,quad) # isolate cover only and rearrange data before saving
...
write_csv(cover, "./AOS/BryoidSurvey/clean_data/AOS_bryoid_cover_1981_1984.csv")
